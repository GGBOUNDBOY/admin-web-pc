...
try {
    ...
    # ---------- 1b. inject copy timestamp into the generated file ----------
    $ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    $content = [System.IO.File]::ReadAllText($target, [System.Text.Encoding]::UTF8)
    $stampTag = "<meta name=`"generated-at`" content=`"$ts`" />`n<!-- generated by publish.ps1 at $ts -->"

    if ($content -match '<!--BUILD_TIME-->') {
        $content = $content -replace '<!--BUILD_TIME-->', $stampTag
    } elseif ($content -match '</head>') {
        $content = $content -replace '</head>', ($stampTag + "`n</head>")
    } elseif ($content -match '</html>') {
        $content = $content -replace '</html>', ($stampTag + "`n</html>")
    } else {
        $content = $content + "`n" + $stampTag
    }

    [System.IO.File]::WriteAllText($target, $content, $utf8NoBom)
    Write-Ok "Injected timestamp: $ts"
